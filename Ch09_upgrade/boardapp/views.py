...
def index(request, pageindex=None):  # 首頁
    ...
    if pageindex == None:  # 無參數
        ...
        if start >= 0:  # 有前頁資料就顯示
            boardunits = models.BoardUnit.objects.order_by(
...
def post(request):  # 新增留言
    if request.method == "POST":  # 如果是以POST方式才處理
        ...
        if postform.is_valid():  # 通過forms驗證
            ...
            unit = models.BoardUnit.objects.create(
                bname=name, bgender=gender, bsubject=subject, bmail=mail, bweb=web, bcontent=content, bresponse='')  # 新增資料記錄
            unit.save()  # 寫入資料庫
            message = '已儲存...'
